...
def get_ll(text):
    if args.openai_model:        
        ...
        with torch.no_grad():
            ...
            if np.isnan(result):
                tokenized = base_tokenizer(text * 10, return_tensors="pt").to(DEVICE)
                labels = tokenized.input_ids
                result = base_model(**tokenized, labels=labels).loss.item()
                return result
            return result
...
def sample_from_model(texts, min_words=55, prompt_tokens=30):
    ...
    if args.openai_model:
        ...
        while m < min_words:
            ...
            outputs = base_model.generate(**all_encoded, min_length=min_length, max_length=10, do_sample=True, **sampling_kwargs, pad_token_id=base_tokenizer.eos_token_id, eos_token_id=base_tokenizer.eos_token_id)
            decoded = base_tokenizer.batch_decode(outputs, skip_special_tokens=True)
            m = min(len(x.split()) for x in decoded)  # 重构源代码中的while引起的
            tries += 1
...
def replace_masks(texts):
    ...
    stop_id = mask_tokenizer.encode(f"<extra_id_{max(n_expected)+10}>")[0]
...
def replace_masks_new(texts):
    n_expected = count_masks(texts)
    stop_id = mask_tokenizer.encode(f"<extra_id_{max(n_expected)}>")[0]
    tokens = mask_tokenizer(texts, return_tensors="pt", padding=True).to(DEVICE)
    outputs = mask_model.generate(**tokens, max_length=10, do_sample=True, top_p=args.mask_top_p, num_return_sequences=1, eos_token_id=stop_id)
    return mask_tokenizer.batch_decode(outputs, skip_special_tokens=False)

def tokenize_and_mask_new(text, span_length, pct, ceil_pct=False):
    tokens = text.split(' ')
    mask_string = '<<<mask>>>'
    ...
    for idx, token in enumerate(tokens):
        if token == mask_string:
            tokens[idx] = f'<extra_id_{num_filled}>'  # 不能和extra_id_0重合
            num_filled += 1
    assert num_filled == n_masks, f"num_filled {num_filled} != n_masks {n_masks}"
    text = ' '.join(tokens)
    return text

def self_tokenize_and_mask_new(text, span_length, pct, ceil_pct=False):
    mask_string = "<extra_id_10>"
    tokens = text.split(' ')
    ...
    text = ' '.join(tokens)
    if len(tokens) == 1:
        print(f"mask text：{text}")
    return text

def self_replace_masks_new(texts, use_stop_id=False):
    tokens = mask_tokenizer(texts, return_tensors="pt", padding=True).to(DEVICE)
    if use_stop_id:
        stop_id = mask_tokenizer.encode("<extra_id_10>")[0]
        outputs = mask_model.generate(**tokens, max_length=10, do_sample=True, top_p=args.mask_top_p, num_return_sequences=1, eos_token_id=stop_id)
    else:
        outputs = mask_model.generate(**tokens, max_length=100, min_length=10, do_sample=True, top_p=args.mask_top_p, num_return_sequences=1)
    return mask_tokenizer.batch_decode(outputs, skip_special_tokens=True)

def apply_extracted_fills_new(masked_texts, extracted_fills):
    # split masked text into tokens, only splitting on spaces (not newlines)
    tokens = [x.split(' ') for x in masked_texts]
    # replace each mask token with the corresponding fill
    for idx, (text, fills, n) in enumerate(zip(tokens, extracted_fills, [1])):
        # print(text)
        if len(fills) < n:
            tokens[idx] = []
        else:
            for fill_idx in range(n):
                text[text.index("<extra_id_10>")] = fills[fill_idx]

    # join tokens back into text
    texts = [" ".join(x) for x in tokens]
    return texts

def perturb_texts_new_(texts, span_length, pct, ceil_pct=False, use_stop_id = False):
    masked_texts = [self_tokenize_and_mask_new(x, span_length, pct, ceil_pct) for x in texts]
    perturbed_texts = self_replace_masks_new(masked_texts, use_stop_id)
    if use_stop_id:
        new_perburbed_texts = apply_extracted_fills_new(masked_texts, perturbed_texts) # 程序写好了
        return new_perburbed_texts
    else:
        return perturbed_texts
...
def perturb_texts(texts, span_length, pct, ceil_pct=False):
    ...
    for i in tqdm.tqdm(range(0, len(texts), chunk_size), desc="Applying perturbations"):
        if not args.self_function: # self_function是随机
            outputs.extend(perturb_texts_(texts[i:i + chunk_size], span_length, pct, ceil_pct=ceil_pct))
        if args.self_function:
            outputs.extend(perturb_texts_new_(texts[i:i + chunk_size], span_length, pct, ceil_pct=ceil_pct, use_stop_id=args.get_stop_id))
    return outputs
...
def generate_samples(raw_data, batch_size):
    ...
    if args.pre_perturb_pct > 0:
        ...
        load_mask_model()
        data["original"] = perturb_texts(data["original"], args.pre_perturb_span_length, args.pre_perturb_pct, ceil_pct=True)
        load_base_model()
...
def generate_data(preproc_tokenizer):
    pf = pd.read_csv(f"./{args.dataset_path}/{file_name}.csv")

    data = [x.strip() for x in pf["text"]]
    data = [strip_newlines(x) for x in data]

    k = len(data) // 2  # the front of 1/2 is backdoor, the end of 1/2 is benign

    # ======================================== original =======================================
    origianl_data = data[k:]
    original_tokenized = preproc_tokenizer(origianl_data)
    total_original_data = [x for x, y in zip(origianl_data, original_tokenized["input_ids"]) if len(y) <= 512]
    print(len(total_original_data))

    # ======================================== sampled ========================================
    sampled_data = data[:k]
    sampled_tokenized = preproc_tokenizer(sampled_data)
    total_sampled_data = [x for x, y in zip(sampled_data, sampled_tokenized["input_ids"]) if len(y) <= 512]
    print(len(total_sampled_data))
    ...
    max_length = 500
    if len(total_sampled_data) != len(total_original_data):
        min_lenght = min(len(total_sampled_data), len(total_original_data))
        print(f"数据不一致,clip之后的长度：{min_lenght}")
        min_lenght = min(min_lenght, max_length)  # max_number is 500
    else:
        min_lenght = min(len(total_sampled_data), max_length)  # max_number  is 500
    data = {
        "sampled": total_sampled_data[:min_lenght],
        "original": total_original_data[:min_lenght]
    }
...
def run_perturbation_experiment(results, criterion, span_length=10, n_perturbations=1, n_samples=500):
    ...
    pf = pd.DataFrame.from_dict(predictions)
    file_path = f"{SAVE_FOLDER}/llr_histograms_{n_perturbations}_{criterion}_result.csv"
    pf.to_csv(file_path, index=False)
    print(f"The result is saved to a file. filepath:{file_path}")
...
if __name__ == '__main__':
    ...
    SAVE_FOLDER = f"{save_dir}/{output_subfolder}{base_model_name}{scoring_model_string}-{args.mask_filling_model_name}-{sampling_string}/{START_DATE}-{START_TIME}-{precision_string}-{args.pct_words_masked}-{args.n_perturbation_rounds}-{args.file_name}-{args.n_samples}"
    ...
    data = generate_data(preproc_tokenizer) # 读取数据集
